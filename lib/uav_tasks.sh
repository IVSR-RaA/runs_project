...
start_uav_runtime() {
  ...
  send_logged "$SESSION:01_uav_run.0" "uav_roscore" "$COMMON_SETUP" "$UAV_ROS_ENV" "roscore -p 11313" false
  send_logged "$SESSION:01_uav_run.1" "uav_mocha_iris" "$COMMON_SETUP" "$UAV_ROS_ENV" "test -f '$PX4_UAV_SDF' && roslaunch mocha_launch iris.launch $MOCHA_ROBOT_CONFIG_ARG sdf:='$PX4_UAV_SDF' world:='$SIM_WORLD_FILE' gui:=false"
  send_logged "$SESSION:01_uav_run.2" "uav_super_lio" "$COMMON_SETUP" "$UAV_ROS_ENV" "$WAIT_FOR_UAV_SENSORS; roslaunch super_lio velodyne_16.launch lidar_topic:=$UAV_LIDAR_TOPIC imu_topic:=$UAV_IMU_TOPIC rviz:=false"
  send_logged "$SESSION:01_uav_run.3" "uav_tf_tree" "$COMMON_SETUP" "$UAV_ROS_ENV" "roslaunch super_lio_lamp_adapter robot_tf.launch robot_name:=$UAV_LAMP_ROBOT odom_topic:=$UAV_ODOM_TOPIC world_to_map_x:=0 world_to_map_y:=0 world_to_map_z:=0"
  send_logged "$SESSION:01_uav_run.4" "uav_vae_keyframes" "$VAE_SETUP" "$UAV_ROS_ENV" "$WAIT_FOR_UAV_LIO_OUTPUTS; roslaunch super_lio_lamp_adapter local_vae_keyframe_pipeline.launch robot_namespace:=$UAV_MOCHA_ROBOT robot_id:=$UAV_MOCHA_ROBOT robot_type:=aerial point_cloud_topic:=$UAV_VAE_POINT_TOPIC odom_topic:=$UAV_ODOM_TOPIC keyframe_vae_topic:=/keyframe_vae"
  send_logged "$SESSION:01_uav_run.5" "uav_local_lamp_input" "$COMMON_SETUP" "$UAV_ROS_ENV" "$WAIT_FOR_UAV_LIO_OUTPUTS; roslaunch super_lio_lamp_adapter local_lio_to_lamp.launch robot_namespace:=$UAV_LAMP_ROBOT robot_id:=$UAV_MOCHA_ROBOT robot_prefix:=$UAV_LAMP_PREFIX odom_topic:=$UAV_ODOM_TOPIC cloud_topic:=$UAV_CLOUD_BODY_TOPIC fixed_frame_id:=$WORLD_FRAME"
  if [[ "$RUN_ONLY" == "all" ]]; then
    send_logged "$SESSION:01_uav_run.6" "uav_receive_ugv" "$VAE_SETUP" "$UAV_ROS_ENV" "roslaunch super_lio_lamp_adapter received_keyframe_vae_to_lamp.launch source_mocha_robot:=$UGV_MOCHA_ROBOT source_robot_id:=$UGV_MOCHA_ROBOT lamp_robot_namespace:=$UGV_LAMP_ROBOT robot_prefix:=$UGV_LAMP_PREFIX robot_type:=ground target_frame:=$WORLD_FRAME sensor_frame:=$UGV_LIDAR_FRAME output_namespace:=$UGV_LAMP_ROBOT/reconstructed"
  fi
  if [[ "$LAMP_MODE" == "distributed" ]]; then
    local fusion_pane=6
    local base1_relays
