...
start_uav_runtime() {
  ...
  if [[ "$LAMP_MODE" == "distributed" ]]; then
    send_plain "$SESSION:01_uav_run.5" "$VAE_SETUP" "$UAV_ROS_ENV" "roslaunch super_lio_lamp_adapter received_keyframe_vae_to_lamp.launch source_mocha_robot:=$UAV_MOCHA_ROBOT source_robot_id:=$UAV_MOCHA_ROBOT lamp_robot_namespace:=$UAV_LAMP_ROBOT robot_prefix:=$UAV_LAMP_PREFIX robot_type:=aerial keyframe_vae_topic:=/$UAV_MOCHA_ROBOT/keyframe_vae target_frame:=$WORLD_FRAME sensor_frame:=$UAV_LIDAR_FRAME output_namespace:=$UAV_LAMP_ROBOT/reconstructed_local"
  else
