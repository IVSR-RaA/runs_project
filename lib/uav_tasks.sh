...
start_uav_runtime() {
  ...
  send_plain "$SESSION:01_uav_run.1" "$COMMON_SETUP" "$UAV_ROS_ENV" "test -f '$PX4_UAV_SDF' && roslaunch mrm_run_launch uav_sim_mocha.launch robot_name:=$UAV_MOCHA_ROBOT $MOCHA_ROBOT_CONFIG_ARG sdf:='$PX4_UAV_SDF' world:='$SIM_WORLD_FILE' gui:=false x:=$UAV_SPAWN_X y:=$UAV_SPAWN_Y z:=$UAV_SPAWN_Z R:=$UAV_SPAWN_ROLL P:=$UAV_SPAWN_PITCH Y:=$UAV_SPAWN_YAW enable_camera:=$UAV_ENABLE_CAMERA"
  send_plain "$SESSION:01_uav_run.2" "$COMMON_SETUP" "$UAV_ROS_ENV" "$WAIT_FOR_UAV_SENSORS; roslaunch super_lio velodyne_16.launch config_file:='$UAV_LIO_CONFIG_FILE' lidar_topic:=$UAV_LIDAR_TOPIC imu_topic:=$UAV_IMU_TOPIC rviz:=false"
  send_plain "$SESSION:01_uav_run.3" "$COMMON_SETUP" "$UAV_ROS_ENV" "roslaunch super_lio_lamp_adapter robot_tf.launch robot_name:=$UAV_LAMP_ROBOT odom_topic:=$UAV_ODOM_TOPIC world_to_map_x:=$UAV_MAP_X world_to_map_y:=$UAV_MAP_Y world_to_map_z:=$UAV_MAP_Z stamp_odom_tf_with_now:=true"
