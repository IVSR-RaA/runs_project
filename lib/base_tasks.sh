...
start_base_runtime() {
  ...
  send_plain "$SESSION:04_base_run.2" "$COMMON_SETUP" "$BASE_ROS_ENV" "roslaunch lamp turn_on_lamp_base.launch robot_namespace:=base1 robot_names_config:='$BASE_LAMP_ROBOT_NAMES_CONFIG' rssi_parameters_config:='$BASE_RSSI_PARAMETERS_CONFIG' run_loop_closure_batcher:=$RUN_GNN_BATCHER publish_map_only_with_subscribers:=$BASE_LAMP_PUBLISH_MAP_ONLY_WITH_SUBSCRIBERS $LAMP_SOLID_ARGS"
