...
SIM_WORLD_FILE="${SIM_WORLD_FILE:-$HOME/PX4-Autopilot/Tools/simulation/gazebo-classic/sitl_gazebo-classic/worlds/nlg_full.world}"
...
UGV_LIDAR_Z_OFFSET="${UGV_LIDAR_Z_OFFSET:-0}"
UGV_ENSURE_JACKAL_CONTROLLER="${UGV_ENSURE_JACKAL_CONTROLLER:-true}"
UGV_GAZEBO_MODEL_NAME="${UGV_GAZEBO_MODEL_NAME:-jackal}"
UGV_JACKAL_CONTROLLER_NAME="${UGV_JACKAL_CONTROLLER_NAME:-jackal_velocity_controller}"
UGV_JACKAL_CONTROLLER_TIMEOUT="${UGV_JACKAL_CONTROLLER_TIMEOUT:-180}"
...
HUSKY_LIDAR_Z_OFFSET="${HUSKY_LIDAR_Z_OFFSET:-0}"
HUSKY_ENSURE_CONTROLLER="${HUSKY_ENSURE_CONTROLLER:-true}"
HUSKY_GAZEBO_MODEL_NAME="${HUSKY_GAZEBO_MODEL_NAME:-husky}"
HUSKY_CONTROLLER_MANAGER_NAMESPACE="${HUSKY_CONTROLLER_MANAGER_NAMESPACE:-husky}"
HUSKY_CONTROLLER_NAME="${HUSKY_CONTROLLER_NAME:-husky_velocity_controller}"
HUSKY_CONTROLLER_TIMEOUT="${HUSKY_CONTROLLER_TIMEOUT:-180}"
...
ENABLE_RGB_CAMERAS="${ENABLE_RGB_CAMERAS:-true}"
...
UAV_LIO_CONFIG_FILE="${UAV_LIO_CONFIG_FILE:-$WS/src/Super-LIO/src/super_lio/config/velody_64_uav.yaml}"
UGV_LIO_CONFIG_FILE="${UGV_LIO_CONFIG_FILE:-$WS/src/Super-LIO/src/super_lio/config/velodyne_16_ugv.yaml}"
HUSKY_LIO_CONFIG_FILE="${HUSKY_LIO_CONFIG_FILE:-$WS/src/Super-LIO/src/super_lio/config/velodyne_16_husky.yaml}"
...
# VAE model/config type. The PX4 Tarot UAV uses the aerial 64-ray VAE config;
# UGV and Husky keep the ground/VLP-16 VAE config.
UAV_VAE_ROBOT_TYPE="${UAV_VAE_ROBOT_TYPE:-aerial}"
...
# Keep the basestation LAMP map latched even if RViz/rostopic subscribes after
# the latest map update. Otherwise /base1/lamp/octree_map can appear silent.
BASE_LAMP_PUBLISH_MAP_ONLY_WITH_SUBSCRIBERS="${BASE_LAMP_PUBLISH_MAP_ONLY_WITH_SUBSCRIBERS:-false}"
...
UGV_CMU_MAX_SPEED="${UGV_CMU_MAX_SPEED:-3.0}"
UGV_CMU_AUTONOMY_SPEED="${UGV_CMU_AUTONOMY_SPEED:-0.4}"
UGV_CMU_WAYPOINT_SPEED="${UGV_CMU_WAYPOINT_SPEED:-0.4}"
UGV_CMU_VEHICLE_LENGTH="${UGV_CMU_VEHICLE_LENGTH:-0.6}"
UGV_CMU_VEHICLE_WIDTH="${UGV_CMU_VEHICLE_WIDTH:-0.6}"
UGV_CMU_CHECK_ROT_OBSTACLE="${UGV_CMU_CHECK_ROT_OBSTACLE:-false}"
UGV_CMU_PATH_SCALE="${UGV_CMU_PATH_SCALE:-1.25}"
UGV_CMU_MIN_PATH_SCALE="${UGV_CMU_MIN_PATH_SCALE:-0.75}"
UGV_CMU_PATH_SCALE_BY_SPEED="${UGV_CMU_PATH_SCALE_BY_SPEED:-true}"
UGV_CMU_WAYPOINT_FILE="${UGV_CMU_WAYPOINT_FILE:-}"
UGV_CMU_BOUNDARY_FILE="${UGV_CMU_BOUNDARY_FILE:-}"
UGV_CMU_WAYPOINT_RADIUS="${UGV_CMU_WAYPOINT_RADIUS:-0.5}"
UGV_CMU_WAYPOINT_Z_BOUND="${UGV_CMU_WAYPOINT_Z_BOUND:-50.0}"
UGV_CMU_WAYPOINT_WAIT="${UGV_CMU_WAYPOINT_WAIT:-0.0}"
UGV_CMU_RUN_MONITOR="${UGV_CMU_RUN_MONITOR:-true}"
...
HUSKY_CMU_MAX_SPEED="${HUSKY_CMU_MAX_SPEED:-10.0}"
HUSKY_CMU_AUTONOMY_SPEED="${HUSKY_CMU_AUTONOMY_SPEED:-3.0}"
HUSKY_CMU_WAYPOINT_SPEED="${HUSKY_CMU_WAYPOINT_SPEED:-3.0}"
HUSKY_CMU_VEHICLE_LENGTH="${HUSKY_CMU_VEHICLE_LENGTH:-1.10}"
HUSKY_CMU_VEHICLE_WIDTH="${HUSKY_CMU_VEHICLE_WIDTH:-0.75}"
HUSKY_CMU_CHECK_ROT_OBSTACLE="${HUSKY_CMU_CHECK_ROT_OBSTACLE:-true}"
HUSKY_CMU_PATH_SCALE="${HUSKY_CMU_PATH_SCALE:-1.50}"
HUSKY_CMU_MIN_PATH_SCALE="${HUSKY_CMU_MIN_PATH_SCALE:-1.50}"
HUSKY_CMU_PATH_SCALE_BY_SPEED="${HUSKY_CMU_PATH_SCALE_BY_SPEED:-false}"
HUSKY_CMU_WAYPOINT_FILE="${HUSKY_CMU_WAYPOINT_FILE:-}"
HUSKY_CMU_BOUNDARY_FILE="${HUSKY_CMU_BOUNDARY_FILE:-}"
HUSKY_CMU_WAYPOINT_RADIUS="${HUSKY_CMU_WAYPOINT_RADIUS:-0.5}"
HUSKY_CMU_WAYPOINT_Z_BOUND="${HUSKY_CMU_WAYPOINT_Z_BOUND:-50.0}"
HUSKY_CMU_WAYPOINT_WAIT="${HUSKY_CMU_WAYPOINT_WAIT:-0.0}"
HUSKY_CMU_RUN_MONITOR="${HUSKY_CMU_RUN_MONITOR:-true}"
CMU_MISSION_MONITOR_STATUS_PERIOD="${CMU_MISSION_MONITOR_STATUS_PERIOD:-5.0}"
CMU_MISSION_MONITOR_STALE_TIMEOUT="${CMU_MISSION_MONITOR_STALE_TIMEOUT:-8.0}"
CMU_MISSION_MONITOR_PROGRESS_TIMEOUT="${CMU_MISSION_MONITOR_PROGRESS_TIMEOUT:-30.0}"

# UAV PX4 sequence-controller integration. UAV_SEQUENCE_YAML remains the
# low-level fallback, but run_mrm defaults to the readable mission YAMLs below.
...
# Optional unified waypoint/path YAML. A global file may contain robot-specific
# sections; per-robot variables override it. By default the normal run mission
# set is enabled; set USE_DEFAULT_MISSIONS=false for mapping-only smoke tests.
USE_DEFAULT_MISSIONS="${USE_DEFAULT_MISSIONS:-true}"
DEFAULT_UAV_MISSION_FILE="${DEFAULT_UAV_MISSION_FILE:-$WS/run/missions/central_uav.yaml}"
DEFAULT_UGV_MISSION_FILE="${DEFAULT_UGV_MISSION_FILE:-$WS/run/missions/ugv_abandoned.yaml}"
DEFAULT_HUSKY_MISSION_FILE="${DEFAULT_HUSKY_MISSION_FILE:-$WS/run/missions/husky_abandoned.yaml}"
MRM_MISSION_FILE="${MRM_MISSION_FILE:-}"
if [[ "$USE_DEFAULT_MISSIONS" == "true" ]]; then
  UGV_MISSION_FILE="${UGV_MISSION_FILE:-${MRM_MISSION_FILE:-$DEFAULT_UGV_MISSION_FILE}}"
  HUSKY_MISSION_FILE="${HUSKY_MISSION_FILE:-${MRM_MISSION_FILE:-$DEFAULT_HUSKY_MISSION_FILE}}"
  UAV_MISSION_FILE="${UAV_MISSION_FILE:-${MRM_MISSION_FILE:-$DEFAULT_UAV_MISSION_FILE}}"
else
  UGV_MISSION_FILE="${UGV_MISSION_FILE:-$MRM_MISSION_FILE}"
  HUSKY_MISSION_FILE="${HUSKY_MISSION_FILE:-$MRM_MISSION_FILE}"
  UAV_MISSION_FILE="${UAV_MISSION_FILE:-$MRM_MISSION_FILE}"
fi
if [[ -n "$UGV_MISSION_FILE" ]]; then
  UGV_ENABLE_CMU_PLANNER="true"
  UGV_CMU_RUN_WAYPOINTS="true"
fi
if [[ -n "$HUSKY_MISSION_FILE" ]]; then
  HUSKY_ENABLE_CMU_PLANNER="true"
  HUSKY_CMU_RUN_WAYPOINTS="true"
fi
if [[ -n "$UAV_MISSION_FILE" ]]; then
...
UAV_ENABLE_EGO_PLANNER="${UAV_ENABLE_EGO_PLANNER:-true}"
...
UAV_EGO_MAP_SIZE_X="${UAV_EGO_MAP_SIZE_X:-300.0}"
UAV_EGO_MAP_SIZE_Y="${UAV_EGO_MAP_SIZE_Y:-300.0}"
UAV_EGO_MAP_SIZE_Z="${UAV_EGO_MAP_SIZE_Z:-40.0}"
UAV_EGO_MAP_RESOLUTION="${UAV_EGO_MAP_RESOLUTION:-0.2}"
UAV_EGO_MAX_VELOCITY="${UAV_EGO_MAX_VELOCITY:-1.0}"
UAV_EGO_MAX_ACCELERATION="${UAV_EGO_MAX_ACCELERATION:-1.0}"
UAV_EGO_PLANNING_HORIZON="${UAV_EGO_PLANNING_HORIZON:-10.0}"
# EGO obstacle inflation is the UAV safety envelope. The default Tarot SDF
# collision body is about 1.0 x 1.0 x 0.85 m, so 0.5 m horizontal and 0.45 m
# vertical are just large enough to enclose the vehicle body.
UAV_EGO_OBSTACLE_INFLATION="${UAV_EGO_OBSTACLE_INFLATION:-0.5}"
UAV_EGO_OBSTACLE_INFLATION_Z="${UAV_EGO_OBSTACLE_INFLATION_Z:-0.45}"
UAV_EGO_LAMBDA_FITNESS="${UAV_EGO_LAMBDA_FITNESS:-1.0}"
UAV_EGO_LOCAL_UPDATE_RANGE_X="${UAV_EGO_LOCAL_UPDATE_RANGE_X:-5.5}"
UAV_EGO_LOCAL_UPDATE_RANGE_Y="${UAV_EGO_LOCAL_UPDATE_RANGE_Y:-5.5}"
UAV_EGO_LOCAL_UPDATE_RANGE_Z="${UAV_EGO_LOCAL_UPDATE_RANGE_Z:-4.5}"
UAV_EGO_GROUND_HEIGHT="${UAV_EGO_GROUND_HEIGHT:--0.01}"
UAV_EGO_MAX_RAY_LENGTH="${UAV_EGO_MAX_RAY_LENGTH:-4.5}"
UAV_EGO_VIRTUAL_CEIL_HEIGHT="${UAV_EGO_VIRTUAL_CEIL_HEIGHT:-40.0}"
UAV_EGO_VISUALIZATION_TRUNCATE_HEIGHT="${UAV_EGO_VISUALIZATION_TRUNCATE_HEIGHT:-40.0}"
UAV_EGO_SPAWN_TEST_OBSTACLE="${UAV_EGO_SPAWN_TEST_OBSTACLE:-false}"
UAV_EGO_USE_POSITION_SETPOINTS="${UAV_EGO_USE_POSITION_SETPOINTS:-false}"
...
WAIT_FOR_UGV_CMU_INPUTS="until timeout ${LIO_OUTPUT_ECHO_TIMEOUT}s rostopic echo -n 1 \"$UGV_CMU_STATE_TOPIC\" >/dev/null 2>&1 && timeout ${LIO_OUTPUT_ECHO_TIMEOUT}s rostopic echo -n 1 \"$UGV_CMU_SCAN_TOPIC\" >/dev/null 2>&1; do echo \"[wait] UGV CMU planner scan and odom input\"; sleep 1; done"
...
WAIT_FOR_HUSKY_CMU_INPUTS="until timeout ${LIO_OUTPUT_ECHO_TIMEOUT}s rostopic echo -n 1 \"$HUSKY_CMU_STATE_TOPIC\" >/dev/null 2>&1 && timeout ${LIO_OUTPUT_ECHO_TIMEOUT}s rostopic echo -n 1 \"$HUSKY_CMU_SCAN_TOPIC\" >/dev/null 2>&1; do echo \"[wait] Husky CMU planner scan and odom input\"; sleep 1; done"
