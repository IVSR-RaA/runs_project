...
# Keep UAV and UGV on separate Gazebo masters so their ROS topics stay on
# separate ROS masters and do not collide through a shared Gazebo process.
...
#
# Change these values when moving the robots in simulation. The map offsets
# default to the spawn x/y location so Gazebo and LAMP/RViz stay aligned.
...
# Odometry consumed by TF, VAE keyframes, and LAMP input.
#
# In the UAV simulation, Super-LIO publishes /lio/odom for a short time and can
# then diverge to nan. MAVROS local odometry stays finite, while Super-LIO still
# provides /lio/cloud_body for local scan input.
...
WAIT_FOR_UAV_LIO_OUTPUTS='until rostopic list 2>/dev/null | grep -qx "/lio/odom" && rostopic list 2>/dev/null | grep -qx "/lio/cloud_body"; do echo "[wait] UAV Super-LIO outputs"; sleep 1; done'
...
UGV_RVIZ_CONFIG="${UGV_RVIZ_CONFIG:-$WS/run/rviz/ugv_only.rviz}"
